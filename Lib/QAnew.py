...
class QA() :
    ...
    def answer(self, questions, contexts) :
        ...
        encodings = tokenizer(questions, contexts, padding=True)
        ...
        for input_id in encodings.input_ids:
            ...
            for each in tokenizer.convert_ids_to_tokens(input_id) :
                tokens.append(each)
            ...
            sep_index = input_id.index(tokenizer.sep_token_id)	          #separator token index
        ...
        scores = model(torch.tensor(encodings.input_ids), token_type_ids=torch.tensor(segment_ids), attention_mask=torch.tensor(attention_masks));
